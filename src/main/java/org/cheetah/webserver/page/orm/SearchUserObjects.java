...
public class SearchUserObjects extends Page {

    private static Logger logger = LoggerFactory.getLogger(SearchUserObjects.class);
    ...
    public void handle(Request request, Response response) {
        ...
        if (userID == null) {
            logger.error("You must provide a user ID to search for objects");
        }
        ...
        this.debugString.append("userID      : " + userID).append(System.lineSeparator());
        ...
        try {
            ...
            logger.error("Error converting to long : '" + userID + "' " + ex.toString());
        }

        try {
            BlockchainObjectReader.SearchResult searchResult = BlockchainObjectReader.getInstance(user).searchUserObjects(userIDLong, oldVersion, "[[\"Login\",\"=\",\"" + filter + "\"]]", limit, offset, order);
            ...
            if (oldVersion) {
                ...
                for (BlockchainObject object : objectList) {
                    jsonArrayResult.add(BlockchainObject.getJSON(object));
                }
            }
            ...
            jsonResult.replace("MessageValue", jsonSearchResult.toJSONString());

        } catch (Exception ex) {
            jsonResult.put("MessageType", "Error");
            jsonResult.put("MessageValue", "Error searching objects for user: '" + userID + "'");
            logger.error("Error searching objects for user: '" + userID + "'");
            ex.printStackTrace();
        }
